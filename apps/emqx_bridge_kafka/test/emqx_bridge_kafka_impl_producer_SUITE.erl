...
publish_helper(
    ...
    x:show(after_create, emqx_resource:list_instances()),
    Partition = 0,
...
delete_all_bridges() ->
    x:show(delete_all_bridges),
    x:show(connectors, emqx_connector:list()),
    x:show(bridges, emqx_bridge:list()),
    x:show(delete_all_bridges),
    lists:foreach(
        fun(#{name := Name, type := Type}) ->
            x:show(delete_bridge, [Type, Name]),
